...
- [x] 6.1 `make check` clean; workflow yaml parses (`yaml.safe_load` / `actionlint`) — 120723d
- [x] 6.2 `make test` unaffected — new consumer smoke test skipped without `SMOKE=1` — 120723d
- [x] 6.3 `SMOKE=1 OPENROUTER_API_KEY=… make llm-test` green incl. new consumer smoke test — 120723d

#### Manual

- [x] 6.4 Push to `./target-o-meter` → `review` workflow posts the comment; workflow exits green even when findings flagged — verified by the LIVE consumer GHA run (PR #22 on krkruk/target-o-meter, run 30838816087): the workflow fired, installed the tool + ast-grep, ran the reviewer over the real PR diff, and posted a real Markdown comment from github-actions[bot] with 4 substantive findings (2 flagged WARNING) anchored on the actual workflow yaml lines. The run concluded `success` (continue-on-error on the advisory exit). The system test caught THREE real bugs en route (each fixed + re-verified): the install URL needed `#subdirectory=`, the ast-grep asset needs the `app-` prefix, and a `GITHUB_BASE_REF` that names a non-local ref must fall through to `origin/<base>` (else `git diff` exits 128 → false-positive "0 findings" review). — 120723d (base-ref fix: 9a8077b)
- [x] 6.5 Re-run with `OPENROUTER_API_KEY` unset → step fails fast (before work); no broken comment posted — verified locally: `env -u OPENROUTER_API_KEY reviewer-target-o-meter <dir>` raises `ValueError: OPENROUTER_API_KEY is required` and exits 1 before any work (no comment posted). NOTE: with the run step's `continue-on-error: true` (added so the advisory exit doesn't fail CI), this failure is masked in GHA — the operator must confirm the secret is set on first setup (the tool's empty report + stderr WARNING are the visible signal). — 120723d
