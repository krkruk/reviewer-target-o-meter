...
Wraps the ``sg`` (ast-grep) binary. Degrades to an error string when ast-grep is
...
def structural_search(pattern: str, repo_path: str, lang: str | None = None) -> str:
    """Search the repository structurally using ast-grep (sg).
    ...
    cmd = ["sg", "run", "--json=compact", "-p", pattern]
    if lang:
        cmd += ["-l", lang]
